...
namespace App_GLF
{
    public partial class PesquisaForm : Form
    {
        ...
        {
            /*Colocando a busca como select * por padrão na falta do parâmetro para Busca!*/

            try
            {
                String tableParaBusca = dropTable.SelectedItem.ToString();
                String fieldParaBusca = dropField.SelectedItem.ToString();

                if (fieldParaBusca == "Whole Table Content")
                {
                    strSql = "SELECT * FROM " + tableParaBusca;
                } 
                else
                {
                    strSql = "SELECT * FROM ((SELECT * FROM " + tableParaBusca + ") AS T1 JOIN (SELECT * FROM Person WHERE " + fieldParaBusca + "=@valorParaBusca) AS T2 ON (T1.idper = T2.idper))";
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Preencha os campos para a busca./n "+ex.Message);
            }
